...
static esp_err_t lcd_backlight_set(bool on)
{
    ...
    ESP_RETURN_ON_ERROR(gpio_config(&io_conf), TAG, "gpio_config BL failed");
    int level = on ? (LCD_BL_ACTIVE_HIGH ? 1 : 0) : (LCD_BL_ACTIVE_HIGH ? 0 : 1);
    gpio_set_level(LCD_BL_PIN, level);
    return ESP_OK;
}
...
static void touch_read_cb(lv_indev_drv_t *drv, lv_indev_data_t *data)
{
    ...
    // 2) Rotation (degrees: 0/90/180/270)
...
esp_err_t lvgl_driver_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(lcd_backlight_set(false), TAG, "backlight init failed");
    ...
    // Memory Access Control: compute from DISPLAY_ROTATION and mirror options
    ...
    // Base rotation (common mapping for ST77xx/ILI9488 family)
        ...
        madctl |= 0; // no MV/MX/MY by default
    ...
    // Map to controller MX/MY depending on rotation
    #if LCD_MIRROR_X
        #if (DISPLAY_ROTATION == 0)
            madctl ^= MADCTL_MX;
        #elif (DISPLAY_ROTATION == 90)
    ...
    ESP_LOGI(TAG, "Display orient: ROT=%d, MIRX=%d, MIRY=%d -> MADCTL=0x%02X (MV=%d MX=%d MY=%d)",
             (int)DISPLAY_ROTATION, (int)LCD_MIRROR_X, (int)LCD_MIRROR_Y, (unsigned)madctl,
             (madctl & MADCTL_MV) ? 1 : 0, (madctl & MADCTL_MX) ? 1 : 0, (madctl & MADCTL_MY) ? 1 : 0);
