...
// STA 연결을 시도하고, 저장된 설정이 없거나 연결에 실패하면 AP 모드로 전환한다.
// 설정 포털(웹 UI) 기동은 이 함수의 책임이 아니다 — wifi_net과 webui 사이의
// 순환 의존을 피하기 위해 오케스트레이션은 main.c로 옮겼다. 호출자는 리턴 후
// wifi_manager_is_ap_mode()로 결과를 확인하면 된다.
esp_err_t wifi_manager_setup_network(void)
{
    ...
    ESP_LOGI(TAG, "AP mode started. Connect to WiFi '%s' and open http://%s", DEFAULT_AP_SSID, DEFAULT_AP_IP_ADDR);
