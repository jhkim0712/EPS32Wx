...
static esp_err_t api_scan_get_handler(httpd_req_t *req)
{
    ...
    cJSON_Delete(root);
    if (!json)
        return send_simple(req, false, "oom");
...
static esp_err_t api_config_post_handler(httpd_req_t *req)
{
    ...
    if (!buf)
        return send_simple(req, false, "invalid body");

    cJSON *root = cJSON_Parse(buf);
    free(buf);
    if (!root)
        return send_simple(req, false, "invalid json");
